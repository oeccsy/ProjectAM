...
public class PerlinNoiseMarchingSquaresTerrain : MonoBehaviour
{
    [SerializeField]
    ...
    public void Build()
    {
        ...
        MapData mapData = mapDataGenerator.GenerateSimpleMapData(perlinNoise);
        
        Mesh mesh = GenerateMesh(mapData.value);
        mesh.name = "Perlin Noise Marching Squares Terrain";
        meshFilter.sharedMesh = mesh;
        meshCollider.sharedMesh = mesh;

        Debug.Log($"Perlin Noise Marching Squares Terrain: vertices={mesh.vertexCount}, triangles={mesh.triangles.Length / 3}");
    }
    ...
    private Mesh GenerateMesh(float[,] field)
    {
        ...
        for (int row = 0; row < height - 1; row++)
        {
            ...
            {
                MarchingSquares.CellData cellData = marchingSquares.Sample(field, row, col);
